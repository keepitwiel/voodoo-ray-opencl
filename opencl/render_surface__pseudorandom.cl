...
)
{
    ...
    uint ri = random_index_generator(*random_index, i);
            ...
            if (within_bounds(target, dims))
            {
                ...
                if (env_target_type == OPEN_SPACE) {
                        ...
                        for (int k=0;k<kmax;k++) {
                            ...
                            d = diffuse_pseudorandom(u[j], s2);
                            ri = random_index_generator(ri, 1);
                            ...
                            while (flag != TERMINATE) {
                                if (flag == PROPAGATE) {
                                    ...
                                    if (length(intensity_tmp) < INTENSITY_THRESHOLD) {
                                        ...
                                        int3 n = convert_int3(pp) - convert_int3(qq);
                                        d = diffuse_pseudorandom(n, s2);
                                        ri = random_index_generator(ri, 1);
                                        s2 = vload3(ri, random_vector);
                                        flag = PROPAGATE;
                                    }
